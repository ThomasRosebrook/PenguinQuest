...
namespace Game3
{
    public abstract class ParticleSystem : DrawableGameComponent
    {
        ...
        protected void AddParticles(Rectangle where)
        {
            ...
            {
                int index = freeParticles.Dequeue();
                InitializeParticle(ref particles[index], 
                    new Vector2(RandomHelper.NextFloat(where.X, where.X + where.Width), 
                    RandomHelper.NextFloat(where.Y, where.Y + where.Height)));
            }
        }
